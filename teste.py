import numpy as np

# S-box
S_BOX = [
    
]

# Inverse S-box
INV_S_BOX = [
    
]

# Round constants
RCON = [
    [0x01, 0x00, 0x00, 0x00],
    [0x02, 0x00, 0x00, 0x00],
    [0x04, 0x00, 0x00, 0x00],
    [0x08, 0x00, 0x00, 0x00],
    [0x10, 0x00, 0x00, 0x00],
    [0x20, 0x00, 0x00, 0x00],
    [0x40, 0x00, 0x00, 0x00],
    [0x80, 0x00, 0x00, 0x00],
    [0x1b, 0x00, 0x00, 0x00],
    [0x36, 0x00, 0x00, 0x00]
]

def sub_bytes(state, sbox):
    return [[sbox[byte] for byte in row] for row in state]

def shift_rows(state):
    state[1] = np.roll(state[1], -1)
    state[2] = np.roll(state[2], -2)
    state[3] = np.roll(state[3], -3)
    return state

def inv_shift_rows(state):
    state[1] = np.roll(state[1], 1)
    state[2] = np.roll(state[2], 2)
    state[3] = np.roll(state[3], 3)
    return state

def mix_columns(state):
    for i in range(4):
        a = state[:, i]
        b = np.copy(a)
        b = ((b << 1) & 0xfe) ^ (((b >> 7) & 1) * 0x1b)

        state[0][i] = b[0] ^ a[1] ^ b[1] ^ a[2] ^ a[3]
        state[1][i] = a[0] ^ b[1] ^ a[2] ^ b[2] ^ a[3]
        state[2][i] = a[0] ^ a[1] ^ b[2] ^ a[3] ^ b[3]
        state[3][i] = a[0] ^ b[0] ^ a[1] ^ a[2] ^ b[3]
    return state

def inv_mix_columns(state):
    for i in range(4):
        a = state[:, i]
        state[0][i] = gf_mult(a[0], 0x0e) ^ gf_mult(a[1], 0x0b) ^ gf_mult(a[2], 0x0d) ^ gf_mult(a[3], 0x09)
        state[1][i] = gf_mult(a[0], 0x09) ^ gf_mult(a[1], 0x0e) ^ gf_mult(a[2], 0x0b) ^ gf_mult(a[3], 0x0d)
        state[2][i] = gf_mult(a[0], 0x0d) ^ gf_mult(a[1], 0x09) ^ gf_mult(a[2], 0x0e) ^ gf_mult(a[3], 0x0b)
        state[3][i] = gf_mult(a[0], 0x0b) ^ gf_mult(a[1], 0x0d) ^ gf_mult(a[2], 0x09) ^ gf_mult(a[3], 0x0e)
    return state

def gf_mult(x, y):
    r = 0
    for i in range(8):
        if y & 1:
            r ^= x
        hbit = x & 0x80
        x <<= 1
        if hbit:
            x ^= 0x1b
        y >>= 1
    return r % 256

def add_round_key(state, key_schedule, round):
    for i in range(4):
        for j in range(4):
            state[i][j] ^= key_schedule[round * 4 + i][j]
    return state

def key_expansion(key):
    key_columns = 4
    key_schedule = [[] for _ in range(44)]

    for i in range(4):
        key_schedule[i] = [key[4 * i], key[4 * i + 1], key[4 * i + 2], key[4 * i + 3]]

    for i in range(4, 44):
        temp = key_schedule[i - 1]
        if i % key_columns == 0:
            temp = sub_word(rot_word(temp))
            temp = [temp[j] ^ RCON[i // key_columns - 1][j] for j in range(4)]
        key_schedule[i] = [key_schedule[i - 4][j] ^ temp[j] for j in range(4)]

    return key_schedule

def sub_word(word):
    return [S_BOX[byte] for byte in word]

def rot_word(word):
    return word[1:] + word[:1]

def cipher(input_bytes, key, num_rounds):
    state = np.array([list(input_bytes[i:i+4]) for i in range(0, len(input_bytes), 4)])
    key_schedule = key_expansion(key)

    state = add_round_key(state, key_schedule, 0)

    for round in range(1, num_rounds):
        state = sub_bytes(state, S_BOX)
        state = shift_rows(state)
        state = mix_columns(state)
        state = add_round_key(state, key_schedule, round)

    state = sub_bytes(state, S_BOX)
    state = shift_rows(state)
    state = add_round_key(state, key_schedule, num_rounds)

    return state.flatten()

def inv_cipher(input_bytes, key, num_rounds):
    state = np.array([list(input_bytes[i:i+4]) for i in range(0, len(input_bytes), 4)])
    key_schedule = key_expansion(key)

    state = add_round_key(state, key_schedule, num_rounds)

    for round in range(num_rounds - 1, 0, -1):
        state = inv_shift_rows(state)
        state = sub_bytes(state, INV_S_BOX)
        state = add_round_key(state, key_schedule, round)
        state = inv_mix_columns(state)

    state = inv_shift_rows(state)
    state = sub_bytes(state, INV_S_BOX)
    state = add_round_key(state, key_schedule, 0)

    return state.flatten()

# Example usage:
input_bytes = b'\x32\x88\x31\xe0\x43\x5a\x31\x37\xf6\x30\x98\x07\xa8\x8d\xa2\x34'
key = b'\x2b\x7e\x15\x16\x28\xae\xd2\xa6\xab\xf7\xcf\x30\x4b\x24\x58\x59'
num_rounds = 10

encrypted = cipher(input_bytes, key, num_rounds)
decrypted = inv_cipher(encrypted, key, num_rounds)

print("Input:", input_bytes)
print("Encrypted:", encrypted)
print("Decrypted:", decrypted)
